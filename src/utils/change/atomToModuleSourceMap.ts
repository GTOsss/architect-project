import { SourceMapAtom, DefaultParamsMA, TemplateParamsMA } from '../../types/sourceMapAtom';

type AtomToModuleSourceMapParams = {
  map: SourceMapAtom;
  defaultParams: DefaultParamsMA;
  aliases: Record<string, string>;
};

export const atomToModuleSourceMap = ({
  map: sourceMapByAtoms = {},
  defaultParams: allDefaultParams = {},
  aliases = {},
}: AtomToModuleSourceMapParams) => {
  const map = {};
  ...
  Object.entries(sourceMapByAtoms).forEach(([componentName, templates]) => {
    templates.forEach((template) => {
      ...
      let params: TemplateParamsMA = templateIsString ? {} : template[1];
      params = params || {};
